...
function DensityMatrix(C::Matrix{Float64}, Nelec::Int)
    N = size(C, 1)
    P_New = zeros(Float64, N, N)
   
    for i in 1:N
        for j in 1:N
            for k in 1:(Nelec ÷ 2) # Factor of 2 for closed-shell only (change for ROHF)
                P_New[i,j] += C[i,k] * C[j,k]
            end
        end
    end
    return 2*P_New
...
function SCF(Basis::BasisSet, Nelec::Int, PGuess::Matrix{Float64} = zeros(Float64, Basis.BasisSize, Basis.BasisSize), ConvC::Float64 = 1e-10, MaxIter::Int = 1000, to::TimerOutput = nothing)
    ...
    @timeit to "SCF Iterations" begin
