...
function PackPrimitives(Basis::BasisSet)
    ...
    for b in 1:NBasis
        ...
        MOcoeffs[b] = Float32(Basis.BasisFunctions[b].MO_Coeff)
    end
    return MaxPrims, PrimN, PrimL, PrimM, Primζ, PrimNorm, PrimCoeff, PrimYNorm, PrimCount, MOcoeffs
end
...
function KernelComputeAOWithAng!(
    ...
    if idx <= total
        ...
        # multiply by MO coefficient for this basis function
        val_total *= MOcoeffsDev[b]
        AODev[idx] = val_total
    end
    return
end
